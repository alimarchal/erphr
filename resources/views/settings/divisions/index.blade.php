<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Divisions" :createRoute="route('divisions.create')"
            createLabel="Add Division" backRoute="dashboard" />
    </x-slot>
    ...
    <x-filter-section :action="route('divisions.index')">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <x-input id="filter_created_from" name="filter[created_from]" type="date"
                    class="mt-1 block w-full" :value="request('filter.created_from')" />
            </div>

            <div>
                <x-label for="filter_created_to" value="Created To" />
                <x-input id="filter_created_to" name="filter[created_to]" type="date"
                    class="mt-1 block w-full" :value="request('filter.created_to')" />
            </div>
        </div>
    </x-filter-section>
    ...
    ]" emptyMessage="No divisions found." :emptyRoute="route('divisions.create')"
        emptyLinkText="Add a division">
